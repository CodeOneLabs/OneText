...
- **C1 global UI** — a live-service HUD: 40 static and 20 changing labels,
...
SDF/MSDF generation, and 2.10.0 moved rasterization to compute shaders — so the
structural difference these numbers rest on (1.0 grows a list of separate
atlas textures, and a label ends up with several canvas renderers) is a
property of the version measured, not of the engine today.
...
this table at 10–38 KB a frame; it is now 0.9–1.2 KB, slightly under UniText's
1.3–1.7 KB. Both hold one material+texture pair. UniText's atlas grows with the
...
TMP's dynamic atlas costs a 10–16 ms hitch at p99 against 0.5 ms, and its
texture memory grows to 9–20 MB and keeps going, while OneText holds the budget
it was configured with. In C1 the language switch produces a 351.8 ms frame in
TMP and a 6.9 ms one here — **51x**, and twenty-one frames a player watches
disappear. The workload matrix isolates why: with unseen glyphs arriving at
...
3–51x cheaper in dynamic-charset workloads, in a fixed memory budget that does
