...
4. `EnsureDisplayText`: re-parses when `_parsedFrom != _text` (string `==`, so value equality — assigning an equal string still re-parses only because the setter nulled `_parsedFrom`) or `_displayText == null` or `_parsedRich != _richText`; `RichTextParser.Parse(_text, _markup, null, null, null)` if markup is possible, else `_markup.Clear()` and `_displayText = _text`.
